...
const seeds: Seed[] = [
  ...
  {
    number: 30, field: FIELDS.network, subField: 'TCP/IP',
    text: 'MTU（Maximum Transmission Unit）が1,500バイトに設定されたIPv4ネットワークで，2,000バイトのデータをTCPを用いて送信するときに，二つ目に送信されるパケットに含まれるデータは何バイトか。ここで，TCPヘッダー長は20バイト，IPヘッダー長は20バイト，イーサネットのヘッダー長とトレーラー長はそれぞれ14バイトと4バイトとする。また，データを複数パケットに分割して送信するときは，先に送信するパケットに格納できる上限までデータを含めて送信するものとする。',
    choices: ['500', '540', '580', '598'], answer: 'イ',
    reasons: ['MTUにはIPヘッダーも含まれるので，最初のパケットに1,500バイトのデータ全てを格納することはできない。', '最初のパケットのTCPデータは1,500−20−20=1,460バイトなので，残りは2,000−1,460=540バイトである。', 'TCPとIPの両ヘッダーを適切に差し引いた残量ではない。', 'イーサネットのヘッダーとトレーラーはMTUの外側であり，TCPデータの分割量には含めない。'],
    points: ['IPv4のMTUはIPパケット全体の上限であり，TCPデータ量はMTUからIPヘッダーとTCPヘッダーを引いて求める。'], keywords: ['MTU', 'TCP', 'IPv4'],
  },
  {
    ...
    choices: ['10.16.32.54', '10.16.32.55', '10.16.32.117', '10.16.32.118'], answer: 'ウ',
    reasons: ['10.16.32.54はネットワークアドレスより小さく，対象サブネット外である。', '10.16.32.55はネットワークアドレスより小さく，対象サブネット外である。', '/26の範囲は64〜127で，利用可能な最大アドレス126から降順に数えると10番目は117である。', '10.16.32.118は最大利用可能アドレス126から数えて9番目である。'],
    points: ['ネットワークアドレス64とブロードキャストアドレス127は機器に割り当てられない。'], keywords: ['CIDR', 'サブネット', 'IPアドレス'],
  },
  {
    number: 33, field: FIELDS.network, subField: 'アプリケーションプロトコル',
    text: 'TCP/IPネットワークで，データ転送用と制御用とに異なるウェルノウンポート番号が割り当てられているプロトコルはどれか。',
    choices: ['FTP', 'POP3', 'SMTP', 'SNMP'], answer: 'ア',
    reasons: ['FTPは制御用に21番，データ転送用に20番など，異なるポートを使用する。', 'POP3は主に110番ポートを使用し，制御用とデータ転送用を分けない。', 'SMTPは主に25番ポートを使用し，制御用とデータ転送用を分けない。', 'SNMPは要求・応答とトラップでポートが異なるが，データ転送用と制御用というFTPの構成ではない。'],
    points: ['FTPは制御コネクションとデータコネクションを分離する。'], keywords: ['FTP', 'ウェルノウンポート', 'TCP/IP'],
  },
  {
    number: 34, field: FIELDS.network, subField: '電子メール',
    text: '30kバイトのJPEGファイルを電子メールに添付するとき，実際の転送データはおよそ何kバイトになるか。ここで，添付ファイルはMIME base64を用いてエンコーディングされるものとする。',
    choices: ['20', '30', '40', '60'], answer: 'ウ',
    reasons: ['base64では元データより小さくならない。', 'エンコードによってデータ量が増えるので，元の30kバイトのままではない。', 'base64は3バイトを4文字で表すので，30×4/3=40kバイト程度になる。', 'base64による増加率は約4/3であり，2倍にはならない。'],
    points: ['base64エンコード後のデータ量は元データのおよそ4/3になる。'], keywords: ['MIME', 'base64', '電子メール'],
  },
  {
    number: 35, field: FIELDS.security, subField: '暗号技術',
    text: 'デジタル庁，総務省及び経済産業省が策定した「電子政府における調達のために参照すべき暗号のリスト（CRYPTREC暗号リスト）」を構成する暗号リストの説明のうち，適切なものはどれか。',
    choices: ['推奨候補暗号リストとは，CRYPTRECによって安全性及び実装性能が確認された暗号技術のうち，市場における利用実績が十分であるか今後の普及が見込まれると判断され，当該技術の利用を推奨するもののリストである。', '推奨候補暗号リストとは，候補段階に格下げされ，互換性維持目的で利用する暗号技術のリストである。', '電子政府推奨暗号リストとは，CRYPTRECによって安全性及び実装性能が確認された暗号技術のうち，市場における利用実績が十分であるか今後の普及が見込まれると判断され，当該技術の利用を推奨するもののリストである。', '電子政府推奨暗号リストとは，推奨段階に格下げされ，互換性維持目的で利用する暗号技術のリストである。'], answer: 'ウ',
    reasons: ['これは電子政府推奨暗号リストの説明であり，推奨候補暗号リストの説明ではない。', '互換性維持目的で利用する暗号技術は運用監視暗号リストに分類される。', '安全性と実装性能が確認され，利用実績又は普及見込みがある技術を推奨するリストの説明である。', '電子政府推奨暗号リストは格下げされた技術のリストではない。'],
    points: ['CRYPTREC暗号リストは電子政府推奨暗号，推奨候補暗号，運用監視暗号の各リストで構成される。'], keywords: ['CRYPTREC', '電子政府推奨暗号', '暗号技術'],
  },
  {
    number: 36, field: FIELDS.security, subField: 'インジェクション攻撃',
    text: 'Webサーバのログに，攻撃と思われるHTTPリクエストヘッダー「GET /cgi-bin/submit.cgi?user=;cat%20/etc/passwd HTTP/1.1」が記録されていた。このリクエストから推測できる，攻撃者が悪用しようとしていた可能性が高い脆弱性はどれか。ここで，「%20」は空白を意味する。',
    choices: ['HTTPヘッダーインジェクション（HTTP Response Splitting）', 'OSコマンドインジェクション', 'SQLインジェクション', 'クロスサイトスクリプティング'], answer: 'イ',
    reasons: ['レスポンスヘッダーへ改行などを挿入する攻撃を示す内容ではない。', '区切り文字の後にcat /etc/passwdというOSコマンドを実行させようとしている。', 'SQL文を不正に組み立てるための入力ではない。', 'ブラウザ上でスクリプトを実行させる入力ではない。'],
    points: ['外部入力をOSコマンド文字列へ連結すると，任意コマンドを実行されるおそれがある。'], keywords: ['OSコマンドインジェクション', 'HTTP', '脆弱性'],
  },
  {
    number: 37, field: FIELDS.security, subField: 'サイドチャネル攻撃',
    ...
    reasons: ['暗号アルゴリズムの数学的性質を利用する線形解読法である。', '機器の電磁波など実装から漏れる物理情報を観測する攻撃はサイドチャネル攻撃である。', '平文差と暗号文差を利用する差分解読法である。', '全ての鍵を試す総当たり攻撃である。'],
    points: ['サイドチャネル攻撃は処理時間，消費電力，電磁波などの漏えい情報を利用する。'], keywords: ['サイドチャネル攻撃', '電磁波解析', 'IoT'],
  },
  {
    number: 38, field: FIELDS.security, subField: 'PKI',
    text: 'PKIを構成するOCSPを利用する目的はどれか。',
    choices: ['誤って破棄してしまった秘密鍵の再発行処理の進捗状況を問い合わせる。', 'デジタル証明書から生成した鍵情報の交換がOCSPクライアントとOCSPレスポンダーの間で失敗した際，認証状態を確認する。', 'デジタル証明書が失効しているかどうかの状態を問い合わせる。', '有効期限が切れたデジタル証明書の更新処理の進捗状況を確認する。'], answer: 'ウ',
    reasons: ['OCSPは秘密鍵の再発行処理を管理するプロトコルではない。', 'OCSPは鍵交換失敗時の認証状態を確認するためのものではない。', 'OCSPは証明書の失効状態をオンラインで問い合わせるためのプロトコルである。', 'OCSPは証明書更新処理の進捗確認には用いない。'],
    points: ['OCSPレスポンダーへ問い合わせることで，CRL全体を取得せず証明書状態を確認できる。'], keywords: ['OCSP', 'PKI', '証明書失効'],
  },
  {
    number: 39, field: FIELDS.security, subField: '人的セキュリティ',
    text: '欧米などの企業，組織で行われている「セキュリティクリアランス」の事例はどれか。',
    choices: ['インターネットと重要な秘密情報を取り扱う組織内ネットワークとを，物理的に分離する。', '重要な秘密情報へのアクセス権を付与されたアカウントを定期的に調べ，不要な場合は削除する。', '重要な秘密情報を取り扱う部署に配置される予定の個人の適性を評価する。', '離席時に机の上の重要な秘密情報を含む書類や媒体を片付ける。'], answer: 'ウ',
    reasons: ['ネットワークの物理分離の事例である。', '特権アカウントなどのアクセス権管理の事例である。', '秘密情報を扱う資格を付与するため，対象者の信頼性や適性を確認する事例である。', 'クリアデスクの事例である。'],
    points: ['セキュリティクリアランスは機密情報へアクセスする人物の適格性を確認する制度である。'], keywords: ['セキュリティクリアランス', '人的セキュリティ', '機密情報'],
  },
  {
    number: 40, field: FIELDS.security, subField: 'SBOM',
    text: 'ソフトウェアの情報セキュリティ対策のうち，SBOM（Software Bill Of Materials）管理ツールを用いることができるものはどれか。',
    choices: ['ソフトウェアでの利用者認証に用いるアカウントの一元管理', 'ソフトウェアについての脆弱性管理', 'ソフトウェアのソースコードについてのバージョン管理，アクセス制御及び改ざん防止', 'ソフトウェアのソースコードのバックアップとマルウェア混入防止'], answer: 'イ',
    reasons: ['アカウントの一元管理はID管理ツールなどの役割である。', 'SBOMでソフトウェアを構成する部品とバージョンを把握し，既知の脆弱性との照合に利用できる。', 'ソースコードのバージョン管理やアクセス制御は構成管理システムなどの役割である。', 'SBOMはソースコードのバックアップやマルウェア混入防止を直接行うツールではない。'],
    points: ['SBOMはソフトウェアに含まれるコンポーネントの一覧であり，サプライチェーンの脆弱性管理に役立つ。'], keywords: ['SBOM', '脆弱性管理', 'ソフトウェアサプライチェーン'],
  },
]
