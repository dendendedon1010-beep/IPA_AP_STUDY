...
function App() {
  ...
  const currentQuestionId = session?.[index]?.id ?? null
  ...
  return (
    <div className={settings.theme === 'dark' ? 'dark bg-[#101713]' : ''}>
      <div className="mx-auto min-h-screen max-w-[480px] bg-paper shadow-2xl dark:bg-[#101713] dark:text-white">
        ...
        <main ref={contentRef} className={`min-w-0 overflow-x-hidden px-4 pt-[92px] ${session ? 'pb-[calc(138px+env(safe-area-inset-bottom))]' : 'pb-[calc(104px+env(safe-area-inset-bottom))]'}`}>
  ...
  return (
    <div className="w-full min-w-0 max-w-full space-y-4 overflow-x-hidden">
      <SettingCard title="学習目標" icon={Target}>
        <label className="block min-w-0 max-w-full text-xs font-bold text-slate-500">試験予定日<input type="date" value={value.examDate} onChange={event => onChange({ ...value, examDate: event.target.value })} className="mt-2 block h-12 w-full min-w-0 max-w-full rounded-xl border-0 bg-slate-100 px-3 font-medium text-ink outline-none dark:bg-white/10 dark:text-white" /></label>
        <label className="mt-4 block min-w-0 max-w-full text-xs font-bold text-slate-500">1日の目標学習時間<div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">{[15, 30, 45, 60].map(minutes => <button key={minutes} onClick={() => onChange({ ...value, dailyMinutes: minutes })} className={`h-11 min-w-0 rounded-xl text-xs font-bold ${value.dailyMinutes === minutes ? 'bg-moss text-white' : 'bg-slate-100 text-slate-500 dark:bg-white/10'}`}>{minutes}分</button>)}</div></label>
      </SettingCard>
...
function SettingCard({ title, icon: Icon, children }: { title: string; icon: typeof Home; children: React.ReactNode }) {
  return <section className="w-full min-w-0 max-w-full overflow-hidden rounded-[24px] bg-white p-5 shadow-sm dark:bg-white/5"><div className="mb-5 flex items-center gap-2 font-bold"><Icon size={18} className="text-moss dark:text-lime" />{title}</div>{children}</section>
}
